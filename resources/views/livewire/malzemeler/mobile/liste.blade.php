            ...
            <div class="modal-body p-0">
              ...
              <div class="tab-content p-0">
                <div class="tab-pane fade show active p-2" id="tab_talep" role="tabpanel">
                  ...
                  <b class="text-danger"
                     style="font-weight: bolder;">{{ $malzeme->stock_name }}</b><br>
                  ...
                  @if ($malzeme_photos)
                    ...
                    @foreach ($malzeme_photos as $p)
                      <img src="{{ asset('public/storage/images/items/thumb/' . $p->foto_path) }}" alt="image"
                           class="imaged m-1 border" style="width:160px">
                    @endforeach
                  @endif
                        ...
                        <tbody>
                          @foreach ($talep_listesi as $d)
                            ...
                            <tr>
                              <td>
                                <span
                                      class="text-danger"><b>{{ $item->stock_name }}</b></span>
                                <br>
      ...
      <div style="justify-content: flex-end;">
        @if ($talep_listesi->count() > 0)
          <span class="badge badge-warning"
                style="margin-right:-10px; position: relative;">{{ $talep_listesi->count() }}</span>
            ...
            <div class="m-1">
              @if ($photo)
                <img src="{{ asset('public/storage/images/items/thumb/' . $photo->foto_path) }}"
                     class="imaged w48 border">
              @else
                <img src="/mobile_assets/img/sample/photo/2.jpg" class="imaged w48 border">
              @endif
            </div>
            <div class="in">
              <div>
      ...
      @if ($malzemeler->hasMorePages())
        ...
        @if ($nfc_btn)
          <button id="scanButton" @if ($nfc_active) disabled @endif class="btn btn-success">
            @if ($nfc_active)
              <ion-icon name="scan" wire:ignore></ion-icon> NFC SCAN Aktif
            @else
              <ion-icon name="scan" wire:ignore>
              </ion-icon> NFC SCAN
            @endif
          </button>
        @endif
